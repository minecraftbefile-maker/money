  ...
  for attempt in range(max_retries):
    try:
      ...
      if attempt == max_retries - 1:
        logging.error(f'[Fatal Error] Connection attempts failed.')
        raise e
...
def render_video_ffmpeg(bg_video, main_audio, output_video):
  # التعديل الحاسم هنا: إجبار FFmpeg على دمج الصوت الجديد بوضوح وتجاهل صمت الفيديو الأصلي
  cmd = [
      FFMPEG_PATH,
      '-y',
      '-i',
...
# High-quality neural voices for each language via Edge TTS
EDGE_VOICES = {
    'ar': 'ar-EG-SalmaNeural',
    'en': 'en-US-ChristopherNeural',
    'es': 'es-ES-AlvaroNeural',
...
# القائمة الكاملة لنماذج النصوص والتفكير المجانية
FREE_CHAT_MODELS = [
    'nvidia/nemotron-3.5-lightning:free',
...
def make_request_with_proxy_rotation(method, url, **kwargs):
  ...
  for attempt in range(max_retries):
    try:
      ...
      logging.warning(
          f'[Network/DNS Retry {attempt+1}/{max_retries}] Failed for'
          f' {url}: {e}'
      )
      if attempt == max_retries - 1:
        logging.error(
            f'[Fatal Error] All {max_retries} connection attempts failed.'
        )
        raise e
      sleep_time = 5 * (attempt + 1)
  ...
  for attempt in range(3):
    try:
      ...
      if response.status_code == 200:
        ...
        return
      logging.warning(
          f'[Fish Audio Attempt {attempt+1}] Status {response.status_code}:'
          f' {response.text}'
      )
    except Exception as e:
      logging.warning(f'[Fish Audio Attempt {attempt+1}] Exception: {e}')
    time.sleep(2)
  raise Exception('Failed to generate TTS audio after 3 attempts.')
...
def download_and_prepare_background():
  ...
  if response.status_code != 200:
    raise Exception(
        'Failed to download background video, status code:'
